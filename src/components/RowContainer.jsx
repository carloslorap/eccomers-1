    ...
    <div
     ...
     className={`w-full my-12 flex items-center gap-3 scroll-smooth ${flag ? 'overflow-x-scroll scrollbar-none':'overflow-x-hidden flex-wrap justify-center'}`}>
        {data && data.length > 0  ? data.map((item)=>(
            <div key={item?.id} className='w-300 h-[255px] min-w-[300px] md:w-340 md:min-w[340px] my-12 bg-cardOverlay rounded-lg p-2 backdrop-blur-lg hover:drop-shadow-lg flex flex-col items-center justify-between'>
            ...
            <div className='w-full flex flex-col gap-4 items-end justify-end'>
                ...
                <p className='mt-auto text-sm text-gray-500'>{item?.calories} Calories</p>
            ...
            <p className='text-xl text-headingColor font-semibold my-10'>Items Not Available</p>
            </div>}
    </div>
  )
}
